...
//  A C++ cross-platform single-header library implementation for universally
//  unique identifiers, simply know as either UUID or GUID (mostly on Windows).
//  A UUID is a 128-bit number used to uniquely identify information in computer
//  systems, such as database table keys, COM interfaces, classes and type
//  libraries, and many others.
//
//  proposed by Marius Bancila
//  https://github.com/mariusbancila/stduuid
//
//  paper:  https://github.com/mariusbancila/stduuid/blob/master/P0959.md

#include <string>
#include <sstream>
#include <iomanip>
#include <array>
//KU #include <string_view>
#include <iterator>
#include <random>
#include <memory>
#include <functional>
...
//KU        constexpr inline unsigned char hex2char(TChar const ch)
        ...
        template <typename TChar>
//KU        constexpr inline bool is_hex(TChar const ch)
        ...
        template <typename TChar>
//KU        constexpr inline unsigned char hexpair2char(TChar const a, TChar const b)
        ...
        public:
            ...
            self_type & operator+=(difference_type const offset)
            {
                if (static_cast<difference_type>(index) + offset < 0 ||
                    static_cast<difference_type>(index) + offset > static_cast<difference_type>(uuid::state_size))
...
//KU        explicit uuid(std::string_view str)
        explicit uuid(std::string const & str)
        {
            create(str.data(), str.size());
        }

//KU        explicit uuid(std::wstring_view str)
...
//KU        constexpr uuid_version version() const noexcept
...
//KU        constexpr bool is_nil() const noexcept
...
inline
    ...
    // KU
    //
    //  Beware of MinGW_64 bug https://sourceforge.net/p/mingw-w64/bugs/338/
    //
    //  std::random_device always delivers the same sequence
...
//KU        uuid operator()(std::string_view name)
        uuid operator()(std::string const & name)
        {
            reset();
...
//KU        uuid operator()(std::wstring_view name)
        uuid operator()(std::wstring const & name)
        {
            reset();
            process_characters(name.data(), name.size());
            return make_uuid();
...
//KU            typename = std::enable_if_t<std::is_integral<char_type>::value>>
            typename bla = typename std::enable_if<std::is_integral<char_type>::value>::type>
